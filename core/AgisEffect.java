    ...
    */
    ...
    public static void removeNonContinuousEffects(CombatInfo target, boolean resume) {
       if(Log.loggingDebug)	Log.debug("AGISEFFECT: removeNonContinuousEffects start "+target.getOid()+" num effects="+target.getCurrentEffects().size());
    	...
    	for (EffectState state : effects) {
			if(Log.loggingDebug)Log.debug("AGISEFFECT: checking to remove effect: " + state.effectName + " it is null? " + state.getEffect()+" resume="+resume);
    		if (resume) {
    	...
    	for (EffectState state : effectsToRemove) {
			...
			if(Log.loggingDebug) Log.debug("EFFECT: unloading effect: " + state.getEffectName());
            try {
                ObjectLockManager.lockAll(requiredLocks);
                if (state.getEffect() == null) {  // broken effect... clean up -- CYC 2009/07/16
            	...
            	Log.error("AgisEffect.removeNonContinuousEffects Exception "+e.getMessage()+" "+e.getLocalizedMessage());
            }
            finally {
                ObjectLockManager.unlockAll(requiredLocks);
            }
	...
	public static class EffectState implements Runnable, Serializable {
		...
		public void resume() {
			...
			if (Log.loggingDebug)
				Log.debug("AgisEffect.resume: effectName=" + effectName + " effect=" + effect + " timeRemaining=" + getTimeRemaining() + " isContinuous? " + effect.isContinuous() + " or passive: " + effect.isPassive());
			effect.resume(this);
        ...
        public String  toString() {
    		return "[EffectState: effectName="+effectName+"; effectID="+effectID+"; effect="+effect+"; abilityID="+abilityID+"; sourceOid="+sourceOid+"; targetOid="+targetOid+"; params="+params+"; item"+item+" RemainingTime="+getTimeRemaining()+"]";
    	}
		...
		protected ArrayList<CoordinatedEffectState> coordinatedEffectStates = new ArrayList<>();
	}
