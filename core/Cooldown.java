...
public class Cooldown implements Serializable, ClientSerializable  {
	...
	public static void activateCooldowns(Collection<Cooldown> cooldowns, CooldownObject obj, int cGlobalMod, int cWeaponMod, int cMod, int attack_speed) {
		activateCooldowns(cooldowns, obj, cGlobalMod, cWeaponMod, cMod, attack_speed, false);
	}

    public static void activateCooldowns(Collection<Cooldown> cooldowns, CooldownObject obj, int cGlobalMod, int cWeaponMod, int cMod , int attack_speed, boolean cNew) {
      ...
      //  LinkedList<Cooldown> cooldownList = (LinkedList<Cooldown>)CombatPlugin.getObjectProperty(obj.getOid(), CombatClient.NAMESPACE, "cooldowns");
        ...
        for (Cooldown cd : cooldowns) {
			...
			if (cd.getStartTime() == 0)
				cd.setStartTime();
			if (cd.getStartTime() + cd.getDuration() < System.currentTimeMillis())
				cd.setStartTime();// Test
			...
			if (cd.getStartTime() > 0) {
				state.setStartTime(cd.getStartTime());
			}
			state.start();
			if (!cooldownList.contains(cd)) {
				if(Log.loggingDebug)
