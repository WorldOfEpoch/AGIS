   ...
   /* public void dispatchMessage(Message message, int flags, MessageCallback callback)
    ...
    private void processInterrupt(boolean force, boolean move , float chance) {
		...
		if (!force && !move && ability.isInterruptible()) {
			...
			float interruptChance = 1f;
			if (CombatPlugin.INTERRUPTION_CHANCE_STAT != null) {
				int statValue = source.statGetCurrentValue(CombatPlugin.INTERRUPTION_CHANCE_STAT);
				if (Log.loggingDebug)
			...
			float interruptRes = 1f;
			if (CombatPlugin.INTERRUPTION_RESISTANCE_STAT != null) {
				int statValue = source.statGetCurrentValue(CombatPlugin.INTERRUPTION_RESISTANCE_STAT);
				if (Log.loggingDebug)
    ...
    private boolean updateStateInternal() {
		...
		try {
			...
			if (CombatPlugin.ABILITY_CAST_TIME_MOD_STAT != null) {
				int statValue = getSource().statGetCurrentValue(CombatPlugin.ABILITY_CAST_TIME_MOD_STAT);
				if (Log.loggingDebug)
				...
				case INIT:
						...
						if (!weaponIsDrawn) {
							if (ability.getDrawnWeaponBefore()) {
								...
								if(response.containsKey("error")){
								 Log.error("DrawWeapon return "+response.get("error"));
								} else {
					...
					if (result != AbilityResult.SUCCESS) {
						...
						for (CoordinatedEffect effect : ability.getPowerUpDefinition(getPowerUptime()).getCoordEffects(ActivationState.INIT)) {
							...
							effect.putArgument("castingMod", Math.round( castingMod * 1000));
					...
					if (!ability.isToggle()) {
						...
						if (acd != null) {
							...
							if (subResult == AbilityResult.SUCCESS) {
								...
								ExtendedCombatMessages.sendCombo(source.getOwnerOid(), ability.getID(), acd.getAbilityId(), acd.getTime(), false);
							} else {
