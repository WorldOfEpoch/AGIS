...
public class MobDatabase {
	...
	public HashMap<Integer, SpawnData> loadInstanceSpawnData(int instanceID) {
		...
		try (PreparedStatement ps = queries.prepare("SELECT * FROM spawn_data where instance = " + instanceID + " AND isactive = 1")) {
    			...
    			if (rs != null) {
    				while (rs.next()) {
    					...
    					/*if (sd.getProperty("alternateSpawnMobTemplate") != null) {
    						// Generate a new copy of the mob factory
    						SpawnData sd2 = new SpawnData();
    					}*/
    				}
    			}
		    }
	...
	public ArrayList<Template> loadMobTemplates(int category) {
		...
		try (PreparedStatement ps = queries.prepare("SELECT * FROM `mob_templates` where category=" + category  + " AND isactive = 1")) {
		    try (ResultSet rs = queries.executeSelect(ps)) {
    			if (rs != null) {
    				while (rs.next()) {
    					int mobID = rs.getInt("id");
    					...
    					mobName.trim();
    					if (Log.loggingDebug)
    						Log.debug(" Load loadMobTemplates "+mobName);
    					...
    					tmpl.put(CombatClient.NAMESPACE, "race", -1);//TODO: Add definiction race for mob in database and editor 
    					tmpl.put(CombatClient.NAMESPACE, "aspect", -1);//TODO Add definiction class/aspect for mob in database and editor
    					tmpl.put(CombatClient.NAMESPACE, "genderId", -1);//TODO Add definiction class/aspect for mob in database and editor
    					...
    					tmpl.put(CombatClient.NAMESPACE, "dmg-max", new AgisStat("dmg-max", dmg_max));
		...
		for (Template tmpl : list) {
			...
			tmpl.put(WorldManagerClient.NAMESPACE, ":statOverrides", loadWmgrMobStats(tmpl.getTemplateID()));
					// Load in mob loot
	...
	private HashMap<String, Integer> loadMobStats(int mobID) {
		...
		try (PreparedStatement ps = queries.prepare("SELECT * FROM `mob_stat` where mobTemplate=" + mobID + " AND isactive = 1")) {
		    try (ResultSet rs = queries.executeSelect(ps)) {
    			if (rs != null) {
    				while (rs.next()) {
	...
	private HashMap<String, Integer> loadWmgrMobStats(int mobID) {
		HashMap<String, Integer> statOverrides = new HashMap<String, Integer>();
		Log.debug("MOB: loading in stat data for mob: " + mobID);
		try (PreparedStatement ps = queries.prepare("SELECT * FROM `mob_stat` where mobTemplate=" + mobID + " AND isactive = 1")) {
		    try (ResultSet rs = queries.executeSelect(ps)) {
    			if (rs != null) {
    				while (rs.next()) {
    					String stat = HelperFunctions.readEncodedString(rs.getBytes("stat"));
    					Log.debug("loadWmgrMobStats stat "+stat+" settings "+AgisWorldManagerPlugin.PROP_MOVEMENT_SPEED+" "+CombatPlugin.STEALTH_STAT+" "+CombatPlugin.PERCEPTION_STEALTH_STAT);
    					Log.debug("loadWmgrMobStats "+(stat == CombatPlugin.STEALTH_STAT)+" or "+(stat.equals(CombatPlugin.STEALTH_STAT)));
    					if(AgisWorldManagerPlugin.PROP_MOVEMENT_SPEED != null && stat.equals(AgisWorldManagerPlugin.PROP_MOVEMENT_SPEED)) {
    				 	...
    				 	Log.debug("loadWmgrMobStats "+statOverrides);
    				}
    			}
			}
		} catch (SQLException e) {
	...
	private HashMap<Integer, MobLootTable> loadMobLoot(int mobID) {
		HashMap<Integer, MobLootTable> lootTables = new HashMap<Integer, MobLootTable>();
		Log.debug("MOB: loading in loot data for mob: " + mobID);
		int id=0;
		try (PreparedStatement ps = queries.prepare("SELECT * FROM `mob_loot` where mobTemplate=" + mobID + " AND isactive = 1 order by dropChance asc")) {
		    try (ResultSet rs = queries.executeSelect(ps)) {
    			if (rs != null) {
    				while (rs.next()) {
	        ...
	        try (ResultSet rs = queries.executeSelect(ps)) {
    			if (rs != null) {
    				while (rs.next()) {
    					...
    					Log.debug("loadProfileBehaviors profile id=" +id + " adding behavior type=" +  type + " weapon="+weapon+" flee_type=" + flee_type + " ability_interval="+ability_interval+" mob_tag="+mob_tag+" ignore_chase_distance="+ignore_chase_distance);
	...
	/*public Map<String, Serializable> loadIconsCurrenciesPrefabData(String objs, OID playerOid) {
    			...
    			if (rs != null) {
    				while (rs.next()) {
    					...
    					if(count > 100000) {
    						props.put("all", false);
    						props.put("num",c);
    						Log.debug("loadIconsCurrenciesPrefabData send part message");
