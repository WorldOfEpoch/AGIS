...
public class AccountDatabase {
	...
	public void deleteOldShopSpwanData() {
		try {
			String updateString = "DELETE FROM `shop_spawn_data` WHERE shop_oid not in (SELECT shop_oid FROM `player_shop`)";
			if(Log.loggingDebug)Log.debug("Delete deleteOldShopSpwanData sql: " + updateString);
	...
	public int getShopSpwanDataId(OID storeOid) {
	...
	public void deleteShopSpwanData(OID storeOid) {
		try {
			String updateString = "DELETE FROM `shop_spawn_data` WHERE shop_oid = "+storeOid.toLong();
			if(Log.loggingDebug)Log.debug("Delete deleteShopSpwanData sql: " + updateString);
			queries.executeUpdate(updateString);
		} catch (Exception e) {
			Log.dumpStack(e.getMessage());
		}
	...
	public ArrayList<Auction> getOwnExpierdAuctions(OID playerOID) {
	...
	public ArrayList<Mail> retrieveMail(OID characterOID, OID accountID) {
		try {
            ...
            try (PreparedStatement ps = queries.prepare("SELECT * FROM " + mailTableName + " WHERE senderId=" + characterOID.toLong() + " AND mailArchive=0 AND expiry < now()")) {
                try (ResultSet rs = queries.executeSelect(ps)) {
        			if (rs != null) {
        				while (rs.next()) {
            ...
            try (PreparedStatement ps = queries.prepare("SELECT * FROM " + mailTableName + " WHERE recipientId=" + characterOID.toLong() + " AND mailArchive=0 AND expiry < now()")) {
                try (ResultSet rs = queries.executeSelect(ps)) {
        			if (rs != null) {
        				while (rs.next()) {
