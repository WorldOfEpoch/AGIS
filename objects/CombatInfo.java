...
//		scheduledExecutioner = 
	...
	protected ConcurrentHashMap<Integer, SkillInfo> skills = new ConcurrentHashMap<>();
	...
	protected Map<Integer, Collection<Integer>> abilities = new ConcurrentHashMap<>();
...
*/
	...
	public void statAddModifier(String statName, Object id, int delta, boolean sendUpdate) {
		...
		try {
			...
			if (stat == null) {
				Log.debug("CombatInfo.statAddModifier: statName=" + statName + " does not exist for this=" + this);
				stat = new AgisStat(statName,0,100000000);
				setProperty(statName,stat);
				if(Log.loggingDebug)
						Log.debug("CombatInfo.statAddModifier: statName=" + statName + " does not exist for this=" + this);
			}
			//else {
			//	int v = stat.getCurrentValue();
				...
				AgisStatDef statDef = CombatPlugin.lookupStatDef(statName);
				if (statDef == null) {
					Log.debug("CombatInfo.statAddModifier: statName=" + statName + " is not registered with CombatPlugin.");
				} else {
					statDef.update(stat, this);
					if (sendUpdate) {
						statSendUpdate(false);
	...
	public void statAddPercentModifier(String statName, Object id, float percent, boolean sendUpdate) {
		...
		try {
			...
			if (stat == null) {
				...
				setProperty(statName,stat);
				if(Log.loggingDebug)
						Log.debug("CombatInfo.statAddPercentModifier: statName=" + statName + " does not exist for this=" + this);
			} 
			//else {
				//int v = stat.getCurrentValue();
	...
	public void statRemoveModifier(String statName, Object id, boolean sendUpdate) {
		...
		try {
			...
			if (stat == null) {
				log.error("statRemoveModifier: stat "+statName+" is null");
				return;
			}
			int v = stat.getCurrentValue();
	...
	public void statRemovePercentModifier(String statName, Object id, boolean sendUpdate) {
		...
		try {
			...
			if (stat == null) {
				log.error("statRemovePercentModifier: stat "+statName+" is null");
				return;
			}
			int v = stat.getCurrentValue();
			if(Log.loggingDebug)
	...
	public void statReapplyModifier(String statName, Object id, int delta, boolean sendUpdate) {
		...
		try {
			...
			if (stat == null) {
				...
				AgisStatDef statDef = CombatPlugin.lookupStatDef(statName);
				if (statDef == null) {
					Log.debug("CombatInfo.statReapplyModifier: statName=" + statName + " is not registered with CombatPlugin.");
				} else {
					statDef.update(stat, this);
					if (sendUpdate) {
						statSendUpdate(false);
	...
	public class ComboTimer implements Runnable {
		...
		public void run() {
			...
			ExtendedCombatMessages.sendCombo(getOwnerOid(), parentAbilityId, abilityId,0, true);
