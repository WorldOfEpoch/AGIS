...
import atavism.agis.objects.LevelExpRequirement;
...
*/
	...
	/*	double mobExpMultiplier = MOB_EXP_RATE_NORMAL;
		...
		if (attacker.isGrouped()) {
			...
			if (oia != null) {
				attakerPos = oia.loc;
				OID targetInstance = WorldManagerClient.getObjectInfo(target.getOid()).instanceOid;
				...
				for (OID member : groupMembers) {
					...
					if (oi != null) {
						...
						if(Log.loggingDebug)log.debug("Group Exp: t: " + target.getOid() + " i: " + targetInstance + " | m: " + member + " i: " + memberInstance);
						if (targetInstance.equals(memberInstance)) {
							...
							if (Point.distanceTo(attakerPos, memberPos) <= EXP_MAX_DISTANCE) {
								...
								if(Log.loggingDebug)log.debug("Group: Member and target distance > " + EXP_MAX_DISTANCE);
							}
						} else {
