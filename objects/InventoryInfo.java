...
public class InventoryInfo extends Entity {
	...
	/*public void setCurrencies(HashMap<Integer, HashMap<Integer, Integer>>curs) {
        lock.lock();
        HashMap<Integer, HashMap<Integer, Long>> cur = new HashMap<Integer, HashMap<Integer, Long>>();
        for (Integer i : curs.keySet()) {
        	HashMap<Integer, Long> c = new HashMap<Integer, Long>();
        	for (Integer cid :curs.get(i).keySet() ) {
        		c.put(cid, (long)curs.get(i).get(cid));
        	}
        	cur.put(i, c);
        }
        
        try {
            this.currencies = new HashMap<Integer, HashMap<Integer, Long>>(cur);
        } finally {
            lock.unlock();
        }
	}*/
	...
	public OID getActiveStorageBag() {
		lock.lock();
        try {
        	if (getStorageBags(currentCategory).containsKey(activeStorage)) {
        		return getStorageBags(currentCategory).get(activeStorage);
        	} else {
        		return null;
        	}
        } finally {
