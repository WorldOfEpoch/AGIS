...
public class AttackAbility extends AgisAbility {
	...
	protected AbilityResult checkAbility(CombatInfo obj, CombatInfo target, ActivationState state, AgisAbilityState abilityState) {

		AbilityResult result = super.checkAbility(obj, target, state, abilityState, false);
		// Check basic results
	...
	void runHitRoll(CombatInfo source, CombatInfo target,AgisAbilityState state) {
		...
		if (hasParry == true) {
			int accuracy = 0, parry = 0;
			if (CombatPlugin.DAMAGE_TYPES.containsKey(damageType) && CombatPlugin.DAMAGE_TYPES.get(damageType).getAccuracyStat() != null) {
				accuracy = source.statGetCurrentValue(CombatPlugin.DAMAGE_TYPES.get(damageType).getAccuracyStat());
			}
			...
			if (CombatPlugin.PARRY_STAT != null) {
				parry = target.statGetCurrentValue(CombatPlugin.PARRY_STAT);
