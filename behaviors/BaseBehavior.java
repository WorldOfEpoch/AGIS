...
public class BaseBehavior extends Behavior implements Runnable {
	...
	public void gotoUpdate() {
		...
		//	if (Log.loggingDebug)
			...
			//	LinkedList<Point> pathPoints = new LinkedList<Point>();
			//	pathPoints.add(myLoc);
			//	WorldManagerClient.MobPathReqMessage correction = new WorldManagerClient.MobPathReqMessage(oid, pathPoints);
		...
		/*
		 * if (moveDebug) { Map<String, Serializable> props = new HashMap<String,
		 * Serializable>(); props.put("x", obj.getWorldNode().getLoc().getX());
		 * props.put("y", obj.getWorldNode().getLoc().getY()); props.put("z",
		 * obj.getWorldNode().getLoc().getZ()); props.put("mob", obj.getOid());
		 * props.put("ext_msg_subtype", "mobposition"); TargetedExtensionMessage TEmsg =
		 * new TargetedExtensionMessage(WorldManagerClient.MSG_TYPE_EXTENSION,
		 * targetDebug, targetDebug, false, props);
	...
	public void followSetup(EntityHandle target, float speed, float distance) {
		...
		if (followObj == null || followObj.getWorldNode() == null) {
			followTarget = null;
			pathState.clear();
			obj.getWorldNode().setDir(new AOVector(0, 0, 0));
			obj.updateWorldNode();
		...
		destLoc = followLoc;
		float followDist = Point.distanceTo(myLoc, followLoc);
		...
		destLoc = _followLoc;
		if (Log.loggingDebug)
			Log.debug("BaseBehavior.followSetup: oid = " + oid + "; myLoc = " + myLoc + "; followLoc=" + followLoc + "; distanceToFollowAt = " + distanceToFollowAt + " mobSpeed=" + mobSpeed + " dist follow=" + followDist);
		scheduleMe(setupPathInterpolator(oid, myLoc, followLoc, true, distanceToFollowAt, node.getFollowsTerrain()));
	...
	public void followUpdate() {
		...
		if (Log.loggingDebug)
			Log.debug("BaseBehavior.followUpdate: oid = " + oid + "; myLoc = " + myLoc + "; followLoc = " + followLoc + "; fdist = " + fdist + "; dist = " + dist + " interpolatingPath=" + interpolatingPath + " "
		...
		if (Log.loggingDebug)
			Log.debug("BaseBehavior.followUpdate: fdist=" + fdist + " dist=" + dist);
		if (dist > 0.1f) {
			if (Log.loggingDebug)
			...
			long msToDest = setupPathInterpolator(oid, node.getLoc(), followLoc, true, distanceToFollowAt, node.getFollowsTerrain());
			if (Log.loggingDebug)
			...
			if (Log.loggingDebug)
				Log.debug("BaseBehavior.followUpdate: msToDest=" + msToDest);
			/*
			 * if(msToSleep < 250L) { if (task != null) task.cancel(true);
