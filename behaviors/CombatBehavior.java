...
public class CombatBehavior extends Behavior {
    ...
    private void handleMovement(PropertyMessage propMsg, OID subject) {
        ...
        if (movementSpeed != null && subject.equals(obj.getOid())) {
            long tStart = System.nanoTime();
            speed = movementSpeed;
	...
	private void handleInvalidPath() {
		...
		if (distance > 0.5f) {
			...
			if (scheduledCheck == null) {
				scheduledCheck = Engine.getExecutor().schedule(cdt, 10000, TimeUnit.MILLISECONDS);
			}
			...
			case 0://melee
				...
				if (reloacatingForAbility) {
					...
					//	selectedAbilityGroup = null;
						...
						long castingTime =  calculateCastingtyime(aa.getActivationTime());
...
//						runAbility(selectedAbility);
    ...
    protected void attackTarget(OID targetOid) {
		...
		try {
			if (Log.loggingDebug)
				Log.debug("CombatBehavior.attackTarget: obj=" + obj.getOid() + " targetOid=" + targetOid + " currentTarget=" + currentTarget + " lastAttacked=" + lastAttacked + " inCombat=" + inCombat +" brhavior="+selectedBehavior);
            ...
            if (targetOid != null) {
                ...
                if (!inCombat) {
                	if (Log.loggingDebug)Log.debug("!!!!!!!!!!!!!! AggroEffect !!!!!!!!!!!!!!!!!!!");
                ...
                goreturn = false;
				if (Log.loggingDebug)
					Log.debug("CombatBehavior.attackTarget: obj=" + obj.getOid() + " scheduledCheck="
							+ (scheduledCheck == null ? "null" : scheduledCheck.isDone() + " " + scheduledCheck.getDelay(TimeUnit.MILLISECONDS) + " " + scheduledCheck.isCancelled()));
				if (scheduledCheck == null) {
					if (Log.loggingDebug)
						Log.debug("CombatBehavior.attackTarget: obj=" + obj.getOid() + " Schedule");
			           scheduledCheck = Engine.getExecutor().schedule(cdt, 1000, TimeUnit.MILLISECONDS);
                }else {
                	if (Log.loggingDebug)
                    	Log.debug("CombatBehavior.attackTarget: obj=" + obj.getOid() + " not Schedule");
              ...
              //  CombatClient.autoAttack(obj.getOid(), null, false);
                ...
                if(Log.loggingDebug)Log.debug("CombatBehavior.attackTarget: loc " + loc + " combatStartLoc=" + combatStartLoc + " centerLoc=" + centerLoc);
		        boolean roamorpatrol = false;
                for (Behavior behav : obj.getBehaviors()) {
        			if (behav instanceof PatrolBehavior) {
        				roamorpatrol = true;
        			}else if(behav instanceof RadiusRoamBehavior) {
        				roamorpatrol = true;
        			}
        		}
                if(behaviorId==-1) {
                ...
                if(Log.loggingDebug)Log.debug("CombatBehavior.attackTarget: command is "+command);
                inCombat = false;
                if (Log.loggingDebug)
	 ...
	 */
	long calculateCastingtyime(long time) {
		float mod = 1f;
	...
	boolean runAbility(int id) {
		...
		if (lastUsedAbility > 0) {
			...
			if (!lastAA.getCastingInRun()) {
				long castingTime = calculateCastingtyime(lastAA.getActivationTime());
...
//                        }
    ...
    private void checkDistance() {
        scheduledCheck = null;
        if(Log.loggingDebug)Log.debug("ComabtBehavior.checkDistance obj="+obj.getOid()+" behaviorProfileId="+behaviorId+" chaseDistance="+chaseDistance+" activated="+activated+" centerLoc="+centerLoc+" currentTarget="+currentTarget);
        if (!activated) {
        	return;
		}
		if (behaviorId == -1) {
			if (currentTarget != null) {
				...
				if (currentTargetObj != null) {
					...
					if(Log.loggingDebug)Log.debug("ComabtBehavior.checkDistance obj=" + obj.getOid() + " distance=" + distance + " distance2=" + distance2 + " distance3=" + distance3);
					if (distance3 > chaseDistance) {
						if(Log.loggingDebug)Log.debug("ComabtBehavior.checkDistance obj=" + obj.getOid() + " remove target form aggro");
					...
					if(Log.loggingDebug)Log.debug("checkDistance: got null ObjectStub during check dostace for oid: " + obj.getOid());
					// return false;
				}
			}
        ...
        if (Log.loggingDebug)
            Log.debug("COMBAT: distance from centerLoc=" + centerLoc + " or combatStartLoc=" + combatStartLoc + "  is " + distance+" chaseDistance="+chaseDistance+" evade="+evade+" command="+command);
		if ((distance > chaseDistance) && (evade == false) && (selectedBehavior != null && selectedBehavior.ignoreChaceDistance == false)) {
            if (Log.loggingDebug)
                ...
                if (!lastAA.getCastingInRun()) {
                    ...
                    if (castingTime + lastUseAbility < System.currentTimeMillis()) {
                        ...
                        for (String effect : effects) {
                            ...
                            if (ae instanceof StunEffect || ae instanceof StunEffect) {
                                foundEffect = true;
                            }
		...
		case 4:// flee
			...
			if (phase == 0) {
				...
				case 2:
					...
					dLoc.multiply(1 / locations.size());
