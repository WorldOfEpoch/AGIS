...
//	 	mobRPCSubId = Engine.getAgent().createSubscription(mobRPCFilter, this, MessageAgent.RESPONDER);
		...
		public boolean processMessage(Message msg, int flags) {
			...
			for (OID oid : members) {
				...
				if (obj != null) {
					...
					if (curWnode.getQuadNode() != null) {
						curWnode.getQuadNode().getTree().updatePerceiver(curWnode.getPerceiver());
					} else
          ...
          //  movement_speed = CombatClient.getPlayerStatValue(masterOid, AgisWorldManagerPlugin.PROP_MOVEMENT_SPEED) ;
           // log.error("UpdateWNodeReqHook: combat movement_speed="+movement_speed+ " " + masterOid + " nanoseconds: " + (System.nanoTime() - start));
			...
			if (newLoc != null) {
        		...
        		else if (!updateOverride && !Points.isClose(oldLoc, newLoc, World.getLocTolerance()) && Point.distanceTo(oldRawLoc, oldLoc)<Point.distanceTo(oldRawLoc, newLoc)) {
				...
				} */else if (!obj.getType().equals(ObjectTypes.mob) && !updateOverride && Point.distanceToXZ(oldLoc, newLoc) > (movement_speed * deltat * 2.5f) && timeDelta > 150/* dirCopy.length() */) {
					if (Log.loggingDebug) {
    ...
    class NoMovePropertyHook implements Hook {
        public boolean processMessage(Message msg, int flags) {
            ...
            QuadTree<WMWorldNode> tree = curWnode != null && curWnode.getQuadNode() != null ? curWnode.getQuadNode().getTree() : null;
			...
			if (CombatPlugin.PERCEPTION_STEALTH_STAT != null && rMsg.containsKey(CombatPlugin.PERCEPTION_STEALTH_STAT)) {
				if (obj != null) {
					...
					if (tree != null) {
                        ...
                        tree.updatePerceiver(curWnode.getPerceiver());
						...
						for (WMWorldNode gainNode : perceivables) {
							if(	curWnode.getObject().getType().isPlayer() && gainNode.getPerceiver() != null)
                                tree.updatePerceiver(gainNode.getPerceiver());
    	...
    	public boolean processMessage(Message msg, int flags) {
			...
			try {
				...
				if (objInfo.objType == ObjectTypes.player) {
					...
					props.put("cmonth", cal.get(Calendar.MONTH));
