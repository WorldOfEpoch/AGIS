...
public class DetourActor implements MessageDispatch {
	...
	public void activate() {
     ...
     //   SubjectFilter responderFilter = new SubjectFilter(oid);
	 //   responderFilter.addType(AgisWorldManagerClient.MSG_TYPE_MOB_FIND_NEAREST_POINT); 
	 //   commandSubResp = Engine.getAgent().createSubscription(responderFilter, this, MessageAgent.RESPONDER);
	...
	void setupFollow(Point pos) {
    	...
    	//Log.error("GeneratePathDragonsan: DeturActor "+obj.getTemplateID()+" node.loc:"+node.getCurrentLoc()+" pos:"+pos+ " lastTargetLoc:"+lastTargetLoc);
    	 //ArrayList<AOVector> generatedPath = navMeshManager.GeneratePathDragonsan(pos, lastTargetLoc);
    	 ArrayList<AOVector> generatedPath = null;
    	 try {
      ...
      Map<String, Serializable> props = new HashMap<String, Serializable>();
       ...
       /*  LinkedList<OID> players = AgisMobClient.GetPlayersOnline(navMeshManager.getInstanceOid());
         for (OID player : players) {
        	  if (Log.loggingDebug) 	Log.debug("DetourActor: setupFollow Sending Path to "+player);
             TargetedExtensionMessage msg = new TargetedExtensionMessage(WorldManagerClient.MSG_TYPE_EXTENSION, player, player, false, props);
         	Engine.getAgent().sendBroadcast(msg);// 000000000001d989 121225
         }*/
         if (Log.loggingDebug) Log.debug("DetourActor: setupFollow Sended Path");
