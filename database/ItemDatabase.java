    			...
    			if (rs != null) {
    				while (rs.next()) {
    					...
    					if (taxPaidUntil != null) {
    						...
    						props.put("i"+c+"setId", loadItemSetForTmplate(id));
						...
						int effnum=0;
    						...
    						props.putAll(getItemRequirementsPrefabData(id,c));
    						c++;
    				}
    			}
			}
...
//						tmpl.put(InventoryClient.ITEM_NAMESPACE, "holsteringWeaponTime", rs.getInt("holsteringWeaponTime"));
    ...
    //							
		...
		for (Template tmpl : list) {
			...
			tmpl.put(InventoryClient.ITEM_NAMESPACE, "item_set", loadItemSetForTmplate(tmpl.getTemplateID()));
		}
		return list;
	}
