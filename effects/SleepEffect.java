...
public class SleepEffect extends AgisEffect {
    ...
    public void apply(EffectState state) {
    	...
    	float sleepChance =1f;
		if(CombatPlugin.SLEEP_CHANCE_STAT!=null) {
			int statValue = source.statGetCurrentValue(CombatPlugin.SLEEP_CHANCE_STAT);
			if (Log.loggingDebug)
		...
		if(CombatPlugin.SLEEP_RESISTANCE_STAT!=null) {
			int statValue = target.statGetCurrentValue(CombatPlugin.SLEEP_RESISTANCE_STAT);
			if (Log.loggingDebug)
			...
			if (CombatPlugin.STAT_THRESHOLDS != null && CombatPlugin.STAT_THRESHOLDS.containsKey(CombatPlugin.SLEEP_RESISTANCE_STAT)) {
				StatThreshold def = CombatPlugin.STAT_THRESHOLDS.get(CombatPlugin.SLEEP_RESISTANCE_STAT);
				int pointsCalculated = 0;
				for (int i = 1; i <= def.getPoints().size(); i++) {
					if (Log.loggingDebug)
