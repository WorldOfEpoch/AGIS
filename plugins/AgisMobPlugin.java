...
//        responderFilter.addType(WorldManagerClient.MSG_TYPE_CHECK_VISIBILITY); 
	...
	void loadData() {
			...
			if (mobForceDespawnInCombat != null) {
				...
				log.debug("CombatPlugin Loaded MOB_FORCE_DESPAWN_IN_COMBAT:"+MOB_FORCE_DESPAWN_IN_COMBAT);
			}
		 ...
		 CombatDatabase coDB = new CombatDatabase(false);
		// Load effects
			ArrayList<AgisEffect> effects = coDB.loadCombatEffects();
			for (AgisEffect effect : effects)
				Agis.EffectManager.register(effect.getID(), effect);
			
			ArrayList<AgisAbility> abilities = coDB.loadAbilities();
			for (AgisAbility ability : abilities) {
				...
				// Load Damage Types
				CombatPlugin.DAMAGE_TYPES = coDB.LoadDamageTypes();
				CombatPlugin.STAT_THRESHOLDS = coDB.LoadStatThresholds();
 ...
 */
    ...
    public static String createMobFactory(SpawnData sd) {
		...
		// Get properties from the template and add/edit behaviours to match
			...
			if (patrolMarkers.size() > 0) {
				...
				//pBehav.setSwinging(behavTmpl.getPatrolSwinging());
		...
		/*	for (int i = 0; i < 3; i++) {
				if (tmpl.getSubMap(CombatClient.NAMESPACE).containsKey("ability" + i)) {
					Integer abilityID = (Integer) tmpl.get(CombatClient.NAMESPACE, "ability" + i);
					if (abilityID != null && abilityID > 0) {
						String statReq = (String) tmpl.get(CombatClient.NAMESPACE, "abilityStatReq" + i);
						float statPercent = (Integer) tmpl.get(CombatClient.NAMESPACE, "abilityStatPercent" + i);
					//	cBehav.addAbility(statReq, statPercent, abilityID);
					}
				}
			}*/
			...
			if (behavTmpl.getIsPlayerCorpse()) {
				...
				if(sd.getProperty("loot_curr")!= null)
					pBehav.setLootCurrency((LinkedList<String>) sd.getProperty("loot_curr"));
			...
			if ( behavTmpl.getIsPlayerShop()) {
				...
				sb.setShopOid((OID)sd.getProperty("shopOid"));
				sb.setShopTimeOut((int)sd.getProperty("shopTimeOut"));
				sb.setShopMessage((String)sd.getProperty("shopMessage"));
				sb.setShopDestroyOnLogOut((boolean)sd.getProperty("destroyOnLogOut"));
				sb.setShopOwner((OID)sd.getProperty("shopOwner"));
				cFactory.addBehav(sb);
			}
		...
		String s = Base64.getEncoder().encodeToString((System.currentTimeMillis()+"").getBytes());
		String factoryName = templateID + "Factory" + numFactories+s;
		if(Log.loggingDebug)
    ...
    public static String createMobShopFactory(SpawnData sd) {
		...
		ShopBehavior psb = new ShopBehavior();
    ...
    class SpawnDomeMobHook implements Hook {
    	public boolean processMessage(Message msg, int flags) {
    		...
    		if (spawnType == -4 /*Dome.MOBTYPE_LOOT*/) {
    			...
    			HashMap<Integer, MobLootTable> lootTables = (HashMap)sd.getProperty("lootTables");
     ...
     * @return
     */
    HashMap<OID, CombatPet> stackcombatPet = new  HashMap<OID, CombatPet>();
    ...
    private boolean spawnCombatPet(int mobID, OID ownerOid, Long duration, int passiveEffect, int skillType) {
    	...
    	CombatPet activePet = null;
    	if (activePetOid != null)
    		if (stackcombatPet.containsKey((OID)activePetOid))
    			activePet = stackcombatPet.get((OID)activePetOid);
    //	NonCombatPet oldPet = (NonCombatPet) EnginePlugin.getObjectProperty(ownerOid, WorldManagerClient.NAMESPACE, "nonCombatPet");
    	if (activePet != null) {
    		activePet.despawnPet();
    		EnginePlugin.setObjectProperty(ownerOid, WorldManagerClient.NAMESPACE, "activePet", null);
    		EnginePlugin.setObjectProperty(ownerOid, WorldManagerClient.NAMESPACE, "hasPet", false);
    		if(Log.loggingDebug)
    	           log.debug("spawnCombatPet berfore spawn Set hasPet false ownerOid="+ownerOid+" mobObj="+activePetOid);
				// If we are trying to spawn the same pet, it means we are actually wanting to despawn the old one, so lets respawn now that it is done
    		if (activePet.getMobTemplateID() == mobID) {
    			Log.debug("PET: despawned old ncPet and now setting the player property to null");
    		    return true;
    		}
    	}else {
    		Log.debug("PET Combat: activePet is null");
