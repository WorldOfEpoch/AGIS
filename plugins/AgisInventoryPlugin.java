    ...
    AgisItem DevModeReloadParamsOfItems(AgisItem ai) {
		...
		if (objMgrProps != null) {
			for (Map.Entry<String, Serializable> entry : objMgrProps.entrySet()) {
				...
				if (!entry.getKey().startsWith(":") && !entry.getKey().equals("enchantLevel")) {
					if(Log.loggingDebug)Log.debug("DevModeReloadParamsOfItems change value for key "+entry.getKey()+" to "+entry.getValue());
            ...
            if (iInfo != null) {
                ...
                for (AgisEquipSlot slot : slotList) {
                    ...
                    if(itemOidToSwitch == null){
                        ...
                        equipBag.removeItem(slot.getId());
                        placeEquippedItem(playerOid, itemOidToSwitch, slot);
...
//                }
    ...
    public SubObjData createInvSubObj(OID mobOid, Template template) {
        ...
        log.debug("InventoryInfo: loaded for ply="+mobOid);
    ...
    protected void createBags(InventoryInfo iInfo, OID mobOid, boolean isPlayer) {
        ...
        if (isPlayer && BANK_SLOTS_COUNT > 0) {
            subBag = createSubBag(iInfo.getOid(), Bag.BAG_USE_TYPE_STORAGE, BANK_SLOTS_COUNT);
            if (subBag == null) {
                return;
            }
        ...
        try {
			...
			for (int i = 0; i < bags.length; i++) {
				...
				if (subBagOid != null) {
					...
					Bag subBag = createSubBag(iInfo.getOid(), Bag.BAG_USE_TYPE_STANDARD, INVENTORY_OTHER_BAG_SIZE);
					if (subBag == null) {
						return;
					}
					added = true;
 ...
 public class lootRollDice implements Runnable {
   ...
   private LinkedList<OID> CalculateLootRoll(OID mobOid,OID itemOid,LinkedList<OID> loot) {
    ...
    public boolean equipItem(AOObject itemObj, OID activatorOid, boolean replace) {
        return equipItem(itemObj, activatorOid, replace, null);
      }
    ...
    public boolean equipItem(AOObject itemObj, OID activatorOid, boolean replace, AgisEquipSlot targetSlot) {
        ...
        try {
                ...
                for (AgisEquipSlot aes : slots) {
                    ...
                    if (oItemOid2 != null) {
                        if (replace) {
                            ...
                            if (!unequipItem(oItemObj, activatorOid, false, null, -1, false, false)) {
            ...
            if (oItemOid != null) {
                ...
                if (!unequipItem(oItemObj, activatorOid, true, sourceBag, sourceSlot, false)) {
...
//            if( Log.loggingDebug) Log.debug("AgisInventoryPlugin.equipItem selected Set"+iInfo.getItemSetSelected()+" slot="+slot.getName()+" set_slot="+set_slot);
...
//            }
    ...
    public boolean unequipItem(AOObject itemObj, OID activatorOid, boolean isReplaced, Bag targetBag, int slotNum, boolean destroyItem) {
        return unequipItem(itemObj, activatorOid, isReplaced, targetBag, slotNum, destroyItem, true);
    }
    /**
        ...
        try {
            ...
            if (hasSpace || (isReplaced && targetBag != null) || destroyItem) {
                ...
                if(set_slot != null){
                    ...
                    removeEquippedItem(activatorOid, item.getOid(), set_slot);
                }else{
...
//        }
    ...
    class LoginHook implements Hook {
        public boolean processMessage(Message msg, int flags) {
            ...
            try {
                ...
                Log.debug("LoginHook:MAIL: loaded " + iInfo.getMail().size() + " pieces of mail for player: " + playerOid);
                ...
                for(String set : sets_name){
                    if(iInfo.getEquippedItemsSetBag(set)==null){
                       Bag subBag = createSubBag(iInfo.getOid(), Bag.BAG_USE_TYPE_EQUIPPED, EQUIP_SLOTS_COUNT);
                        if (subBag != null) {
                ...
                for(PlayerShop ps : list) {
                    ...
                    Log.debug("ShopBehavior: ValidateShop time="+time+" shopCreateTime="+ps.getCreateTime()+" shopTimeOut="+ps.getTimeout()+" shopOwner="+ps.getOwnerOid()+" shopDestroyOnLogOut="+ps.getEndPlayerOnLogout()+" shopOid="+ps.getShopOid());
                    if((ps.getTimeout() > 0 && time > ps.getCreateTime() + ps.getTimeout() * 60000) || ps.getEndPlayerOnLogout()) {
                        ...
                        Log.debug("StopShopHook: storeOid=" + ps.getShopOid()+" items="+listItems.size());
                        ArrayList<OID> itemList = new ArrayList<OID>();
                        int curr=0;
                        long currAmount = 0;
        ...
        public boolean processMessage(Message msg, int flags) {
            ...
            if(iInfo!=null) {
                if (bankId == null || bankId < 0) {
                    ...
                    if (bankBagOid == null) {
                        Bag subBag = createSubBag(iInfo.getOid(), Bag.BAG_USE_TYPE_STORAGE, BANK_SLOTS_COUNT);
                        if (subBag == null) {
                            return false;
                        }
    ...
    class CreateStorageHook implements Hook {
        public boolean processMessage(Message msg, int flags) {
            ...
            if (isPlayerStorage) {
                InventoryInfo iInfo = getInventoryInfo(oid);
                Bag subBag = createSubBag(iInfo.getOid(), Bag.BAG_USE_TYPE_STORAGE, storageSize);
                if (subBag == null) {
                ...
                Bag storageBag = createSubBag(null, Bag.BAG_USE_TYPE_STORAGE, storageSize);
