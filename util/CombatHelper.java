...
public class CombatHelper {
	...
	public static double CalcHitChance(CombatInfo obj, CombatInfo target, int skillType, String dmgType, AgisAbility ability) {
		...
		if (CombatPlugin.DAMAGE_TYPES.containsKey(dmgType) && CombatPlugin.DAMAGE_TYPES.get(dmgType).getAccuracyStat() != null) {
			accuracy = obj.statGetCurrentValue(CombatPlugin.DAMAGE_TYPES.get(dmgType).getAccuracyStat());
		}
		int evasion = 1;
		if (CombatPlugin.DAMAGE_TYPES.containsKey(dmgType) && CombatPlugin.DAMAGE_TYPES.get(dmgType).getEvasionStat() != null) {
			evasion = target.statGetCurrentValue(CombatPlugin.DAMAGE_TYPES.get(dmgType).getEvasionStat());
		}
	...
	public static int CalcDamage(CombatInfo obj, CombatInfo caster, int dmg, String dmgType, float skillMod, int skillType, int hitRoll, boolean useHitRoll, Map<String, Serializable> params) {
		Log.debug("COMBATHELPER: CalcDamage with base dmage: " + dmg + ", about to get attackType");
		if (dmgType == null || dmgType.equals("")) {
			// Get the attackers damageType
			dmgType = (String) caster.getProperty("attackType");
		}
		Log.debug("COMBATHELPER: CalcDamage, just got attackType: " + dmgType);
		double damage = dmg;
		...
		Log.debug("COMBATHELPER: CalcDamage, skillMod: " + skillMod + " skillLevel:" + skillLevel + " skillType:" + skillType);

		damage += skillMod * (float) skillLevel;
		Log.debug("COMBAT: after skill mod dmg: " + damage + "; damage type: " + dmgType);
		Log.debug("COMBAT: POWER_STAT " + CombatPlugin.DAMAGE_TYPES.get(dmgType).getPowerStat() + "; DAMAGE_DEALT_MODIFIER: " + CombatPlugin.DAMAGE_DEALT_MODIFIER);

		if (CombatPlugin.DAMAGE_TYPES.get(dmgType).getPowerStat() != null) {
			double casterStrength = caster.statGetCurrentValue(CombatPlugin.DAMAGE_TYPES.get(dmgType).getPowerStat());
		...
		Log.debug("COMBAT: before hitRoll  calculation dmg: " + damage + "; damage type: " + dmgType);
		Log.debug("COMBAT: hitRoll " + hitRoll + " useHitRoll=" + useHitRoll + " roll=" + roll + " DAMAGE_HITROLL_MODIFIER=" + CombatPlugin.DAMAGE_HITROLL_MODIFIER);
		if (useHitRoll) {
		...
		Log.debug("DMGTYPE: getting resistance stat: " + CombatPlugin.DAMAGE_TYPES.get(dmgType).getResistanceStat() + " for damage type: " + dmgType);
		
		
		
		double targetArmor = 0;
		if(obj!=null)
			targetArmor=  obj.statGetCurrentValue(CombatPlugin.DAMAGE_TYPES.get(dmgType).getResistanceStat());
		Log.debug("COMBAT: Helper targetArmor:" + targetArmor + "; Calcultion Flat bool->" + CombatPlugin.FLAT_ARMOR_DAMAGE_CALCULATIONS);
		...
		if(obj!=null)
			targetDamageModifier = obj.statGetCurrentValue(CombatPlugin.DAMAGE_TAKEN_MODIFIER);
		...
		int criticalChance = caster.statGetCurrentValue(CombatPlugin.DAMAGE_TYPES.get(dmgType).getCriticChanceStat());
		...
		int criticalChanceCalculated = 0;
		if (CombatPlugin.STAT_THRESHOLDS != null && CombatPlugin.STAT_THRESHOLDS.containsKey(CombatPlugin.CRITIC_CHANCE)) {
			...
			for (int i = 1; i <= def.getPoints().size(); i++) {
				Log.debug("COMBAT: Critic i="+i+" pointsCalculated="+pointsCalculated+" Th="+def.getThresholds().get(i)+" points="+def.getPoints().get(i)+" criticalChanceCalculated="+criticalChanceCalculated);
		...
		Log.debug("COMBAT: Critic criticalChanceCalculated: " + criticalChanceCalculated);

		double rand = random.nextDouble();
		Log.warn("COMBAT: Critic chance: " + rand + "; calc chance: " + (criticalChanceCalculated / 100f) + "; calc chance v2: " + (criticalChanceCalculated / 100.0));

		if (rand < (criticalChanceCalculated / 100.0)) {
			float modValue = 0f;
			if (CombatPlugin.DAMAGE_TYPES.get(dmgType).getCriticPowerStat() != null) {
				int criticalPower = caster.statGetCurrentValue(CombatPlugin.DAMAGE_TYPES.get(dmgType).getCriticPowerStat());
				if (Log.loggingDebug)
					Log.debug("COMBAT: Critic stat: " + criticalPower + " of " + CombatPlugin.DAMAGE_TYPES.get(dmgType).getCriticPowerStat());
				int criticalPowerCalculated = 0;
	...
	public static int modifyDamage(CombatInfo caster, CombatInfo target, int dmg) {
		...
		float damageDealtMod = 1;
		if (CombatPlugin.ABILITY_DAMAGE_DEALT_MOD_STAT != null) {
			int statValue = caster.statGetCurrentValue(CombatPlugin.ABILITY_DAMAGE_DEALT_MOD_STAT);
			if (Log.loggingDebug)
		...
		float damageReciveMod = 1;
		if (target != null)
			if (CombatPlugin.ABILITY_DAMAGE_RECEIVE_MOD_STAT != null) {
				int statValue = target.statGetCurrentValue(CombatPlugin.ABILITY_DAMAGE_RECEIVE_MOD_STAT);
				if (Log.loggingDebug)
		...
		return Math.round(dmg * damageReciveMod * damageDealtMod);
	}
	...
	public static int CalcHeal(CombatInfo obj, CombatInfo caster, int heal, float skillMod, int skillType, String dmgType, Map<String, Serializable> params) {
		...
		if (caster != null) {
			if (CombatPlugin.DAMAGE_TYPES.get(dmgType) != null) {
				...
				float healPowerMod = 1;
				if (CombatPlugin.DAMAGE_TYPES.get(dmgType).getPowerStat() != null) {
					int statValue = caster.statGetCurrentValue(CombatPlugin.DAMAGE_TYPES.get(dmgType).getPowerStat());
					if (Log.loggingDebug)
		...
		float healDealtMod = 1;
		if (caster != null) {
			if (CombatPlugin.ABILITY_HEALTH_DEALT_MOD_STAT != null) {
				int statValue = caster.statGetCurrentValue(CombatPlugin.ABILITY_HEALTH_DEALT_MOD_STAT);
				if (Log.loggingDebug)
		...
		float healReciveMod = 1;
		if (CombatPlugin.ABILITY_HEALTH_RECEIVE_MOD_STAT != null) {
			int statValue = obj.statGetCurrentValue(CombatPlugin.ABILITY_HEALTH_RECEIVE_MOD_STAT);
			if (Log.loggingDebug)
		...
		return Math.round(healVal * healDealtMod * healReciveMod);
	}
