		...
		if (useEnterCombatState)
			...
			if (startCooldownsOnActivation) {
				...
				if(CombatPlugin.ABILITY_COOLDOWN_MOD_STAT!=null) {
					int statValue = combatInfo.statGetCurrentValue(CombatPlugin.ABILITY_COOLDOWN_MOD_STAT);
					if (Log.loggingDebug)
				...
				if(CombatPlugin.ABILITY_GLOBAL_COOLDOWN_MOD_STAT!=null) {
					int statValue = combatInfo.statGetCurrentValue(CombatPlugin.ABILITY_GLOBAL_COOLDOWN_MOD_STAT);
					if (Log.loggingDebug)
				...
				if(CombatPlugin.ABILITY_WEAPON_COOLDOWN_MOD_STAT!=null) {
					int statValue = combatInfo.statGetCurrentValue(CombatPlugin.ABILITY_WEAPON_COOLDOWN_MOD_STAT);
					if (Log.loggingDebug)
		...
		if (consumeOnActivation) {
			...
			if (costProp != null) {
				...
				if (statDef instanceof VitalityStatDef) {
					...
					Integer maxValue = combatInfo.statGetCurrentValue(maxStat);
					percentage = Math.round(maxValue * getActivationCostPercentage() / 100f);
				}
				float costMultiply = 1f;
				if (CombatPlugin.ABILITY_COST_MOD_STAT != null) {
					int statValue = combatInfo.statGetCurrentValue(CombatPlugin.ABILITY_COST_MOD_STAT);
					if (Log.loggingDebug)
				...
				combatInfo.statModifyBaseValue(costProp, -Math.round((activationCost + percentage) * costMultiply));
		...
		if (result == AbilityResult.SUCCESS) {
			...
			if (pulseCostProp != null && !pulseCostProp.equals("")) {
				...
				if(pulsStatDef instanceof VitalityStatDef) {
					...
					String maxStat = def.getMaxStat();
					Integer pulsMaxValue  = combatInfo.statGetCurrentValue(maxStat);
					pulsCostPercentage = Math.round(pulsMaxValue * getPulseCostPercentage()/100f);
				}
				combatInfo.statModifyBaseValue(pulseCostProp, -Math.round((pulseCost + pulsCostPercentage) * costMultiply));
				combatInfo.sendStatusUpdate();
			}
		} else {
			AgisAbility.interruptAbility(state, result);
...
//        Log.debug("TARGET: attackable targets: " + caster.getAttackableTargets());
    ...
    protected AbilityResult checkRange(CombatInfo obj, CombatInfo target, BasicWorldNode casterWNode, float rangeTolerance,boolean abilityCheck,AgisAbilityState abilityState) {
		...
		if(CombatPlugin.ABILITY_RANGE_MOD_STAT!=null) {
			int statValue = obj.statGetCurrentValue(CombatPlugin.ABILITY_RANGE_MOD_STAT);
			if (Log.loggingDebug)
	...
	protected AbilityResult checkCost(CombatInfo obj, CombatInfo target, ActivationState state) {
		...
		float costMultiply =1f;
		if(CombatPlugin.ABILITY_COST_MOD_STAT!=null) {
			int statValue = obj.statGetCurrentValue(CombatPlugin.ABILITY_COST_MOD_STAT);
			if (Log.loggingDebug)
				Log.debug("AgisAbility: CostMod statValue: " + statValue + " of " + CombatPlugin.ABILITY_COST_MOD_STAT);
			int calculated = 0;
			if (CombatPlugin.STAT_THRESHOLDS != null && CombatPlugin.STAT_THRESHOLDS.containsKey(CombatPlugin.ABILITY_COST_MOD_STAT)) {
		...
		int costPercentage=0;
		Integer costValue = obj.statGetCurrentValue(costProp);
		AgisStatDef statDef = CombatPlugin.lookupStatDef(costProp);
		if(statDef instanceof VitalityStatDef) {
			VitalityStatDef def = (VitalityStatDef)statDef;
			String maxStat = def.getMaxStat();
			Integer maxMaxValue = obj.statGetCurrentValue(maxStat);
		...
		switch (state) {
		case INIT:
			if ((getActivationCost() + costPercentage) * costMultiply> obj.statGetCurrentValue(costProp)) {
		...
		case CHANNELLING:
			...
			if(statDef instanceof VitalityStatDef) {
				VitalityStatDef def = (VitalityStatDef)pulsstatDef;
				String maxStat = def.getMaxStat();
				Integer pulsMaxValue = obj.statGetCurrentValue(maxStat);
				pulscostPercentage = Math.round(pulsMaxValue * getPulseCostPercentage()/100f);
			}
				if ((getPulseCost() + pulscostPercentage) * costMultiply> obj.statGetCurrentValue(pulseCostProp)) {
				return AbilityResult.INSUFFICIENT_ENERGY;
			}
			break;
		case ACTIVATED:
			...
			if (getActiveCost()*costMultiply > obj.statGetCurrentValue(costProp)) {
				return AbilityResult.INSUFFICIENT_ENERGY;
			}
			break;
		case COMPLETED:
	...
	protected AbilityResult checkPulseCost(CombatInfo obj, CombatInfo target, ActivationState state) {
		...
		int costPercentage=0;
		Integer costValue = obj.statGetCurrentValue(pulseCostProp);
		...
		switch (state) {
		case ACTIVATING:
				if ((getPulseCost() + costPercentage) * costMultiply > obj.statGetCurrentValue(pulseCostProp)) {
					return AbilityResult.INSUFFICIENT_ENERGY;
				}
			break;
		case CHANNELLING:
    	...
    	switch (state) {
    		...
    		case ACTIVATING:
    			if (vigorcost > obj.statGetCurrentValue("vigor")) {
    				if(Log.loggingDebug)Log.debug("ABILITY: vigorcost: " + vigorcost + "current vigor: " + obj.statGetCurrentValue("vigor"));
